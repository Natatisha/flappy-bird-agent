...
class DQN:
    def __init__(self, K, conv_layer_sizes, hidden_layer_sizes, scope):
        ...
        with tf.variable_scope(scope):
            ...
            # final output layer
            self.predict_op = tf.contrib.layers.fully_connected(Z, K, scope='predict_op')
    ...
    def save(self, frame_number, write_meta_graph):
        if self.saver is None:
            self.saver = tf.train.Saver(max_to_keep=4)
        self.saver.save(self.session, 'my_model', global_step=frame_number, write_meta_graph=write_meta_graph)
...
if __name__ == '__main__':
    ...
    PERFORMANCE_SUMMARIES = tf.summary.merge([REWARD_SUMMARY])
    ...
    with tf.Session() as sess:
        ...
        for i in range(num_episodes):
            ...
            summ = sess.run(PERFORMANCE_SUMMARIES, feed_dict={REWARD_PH: last_100_avg})
